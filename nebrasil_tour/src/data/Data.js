const panoramasData = [
  //INICIO 
  {                                                        //0
    url: "../images/comp/ne-beg-1.jpg",
    infospots: [
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [-4534.26, 593.08, 2011.69],
        title: "Tour Virtual",
        index: 1,
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [-2429.06, -4340.71, -457.40],
        title: "Passadiço",
        index: 29,
      },
    ]
  },
  //CONVOO
  {                                                        //1
    url: "../images/comp/ne-conv-1.jpg",
    infospots: [
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [-4421.00, -281.67, 2297.30],
        title: "Sala de Estar dos Guardas-Marinha",
        index: 2,
        
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [4969.53, 467.65, -117.02],
        title: "VOLTAR",
        index: 0,
      }
    ]
  },
  //SALA DE ESTAR DOS GUARDA MARINHA
  {                                                         //2
    url: "../images/comp/ne-gm-1.jpg",
    infospots: [
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [-4990.44, -65.03, -133.62],
        title: "Sala de Estar dos Guardas-Marinha",
        index: 3,
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [4932.76, 792.40, -69.63],
        title: "Convoo do NE Brasil",
        index: 1,
      }
    ]
  },
  {                                                          //3
    url: "../images/comp/ne-gm-2.jpg",
    infospots: [
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [-4937.25, -646.49, 329.09],
        title: "",
        index: 4,
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [-4695.19, -668.81, 1567.48],
        title: "",
        index: 5,
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [4971.85, -49.09, -500.27],
        title: "Convoo do NE Brasil",
        index: 1,
      }
    ]
  },
  {                                                           //4
    url: "../images/comp/ne-gm-3.jpg",
    infospots: [
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [-4074.28, -1480.28, 2478.77],
        title: "",
        index: 5,
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [-4642.69, -86.55, -1828.91],
        title: "",
        index: 6,
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [3976.52, -596.33, 2960.82],
        title: "",
        index: 3,
      }
    ]
  },
  {                                                            //5
    url: "../images/comp/ne-gm-4.jpg",
    infospots: [
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [-4118.23, 206.05, -2811.45],
        title: "",
        index: 6,
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [-4900.01, -428.24, -855.47],
        title: "",
        index: 7,
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [-4853.89, -346.92, 1124.48],
        title: "",
        index: 8,
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [-502.12, 568.16, -4937.46],
        title: "",
        index: 4,
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [4542.90, -673.13, -1962.78],
        title: "",
        index: 3,
      }, 
    ]
  },
  {                                                            //6
    url: "../images/comp/ne-gm-5.jpg",
    infospots: [
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [-606.23, -614.82, 4917.16],
        title: "",
        index: 7,
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [3293.48, -1335.28, 3505.85],
        title: "",
        index: 8,
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [1549.95, -62.97, -4750.77],
        title: "",
        index: 3,
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [3729.77, -1707.50, -2844.90],
        title: "",
        index: 5,
      },
    ]
  },
  {                                                            //7
    url: "../images/comp/ne-gm-6.jpg",
    infospots: [
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [4633.69, 761.16, 1697.23],
        title: "Auditório",
        index: 9,
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [4937.50, -708.49, 184.38],
        title: "Corredor",
        index: 14,
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [1549.95, -62.97, -4750.77],
        title: "",
        index: 3,
      },
    ]
  },
  {                                                            //8
    url: "../images/comp/ne-gm-7.jpg",
    infospots: [
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [1549.95, -62.97, -4750.77],
        title: "",
        index: 3,
      },
    ]
  },
  //AUDITÓRIO
  {                                                             //9
    url: "../images/comp/ne-aud-1.jpg",
    infospots: [
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [260.53, -64.62, 4983.81],
        title: "",
        index: 10,
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [-1416.55, -4701.80, -917.53],
        title: "Sala de Estar dos Guardas-Marinha",
        index: 7,
      },
    ]
  },
  {                                                             //10
    url: "../images/comp/ne-aud-2.jpg",
    infospots: [
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [260.53, -64.62, 4983.81],
        title: "",
        index: 9,
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [-4956.07, -354.98, 538.02],
        title: "",
        index: 11,
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [-4465.07, -49.26, -2230.90],
        title: "",
        index: 12,
      },
    ]
  },
  {                                                             //11
    url: "../images/comp/ne-aud-3.jpg",
    infospots: [
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [-3731.65, -131.20, -3317.81],
        title: "",
        index: 12,
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [-4512.30, -215.20, -2123.86],
        title: "",
        index: 13,
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [4795.25, 110.73, 1379.15],
        title: "",
        index: 10,
      },
    ]
  },
  {                                                             //12
    url: "../images/comp/ne-aud-4.jpg",
    infospots: [
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [-4565.65, -87.60, 2029.49],
        title: "",
        index: 10,
      },
    ]
  },
  {                                                             //13
    url: "../images/comp/ne-aud-5.jpg",
    infospots: [
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [4993.64, 85.41, -57.40],
        title: "Saída do Auditório",
        index: 9,
      },
    ]
  },
  //CORREDOR
  {                                                             //14
    url: "../images/comp/ne-crd-1.jpg",
    infospots: [
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [4985.46, -233.50, 102.45],
        title: "Corredor",
        index: 15,
      },
    ]
  },
  {                                                             //15
    url: "../images/comp/ne-crd-2.jpg",
    infospots: [
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [-4979.42, -54.73, 351.72],
        title: "Camarote",
        index: 16,
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [-525.72, 1862.15, -4606.43],
        title: "Sala de Estar dos Guarda-Marinha",
        index: 7,
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [4981.80, 352.77, 82.72],
        title: "Centro Cirúrgico",
        index: 38,
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [4920.59, 399.49, -747.23],
        title: "Atendimento Odontológico",
        index: 37,
      },
    ]
  },
  {                                                             //16
    url: "../images/comp/ne-crd-4.jpg",
    infospots: [
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [-4078.64, 2.07, -2887.11],
        title: "Praça D'Armas",
        index: 17,
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [-4946.49, 117.46, 658.11],
        title: "Camarote",
        index: 31,
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [2754.15, -133.36, 4158.84],
        title: "Voltar",
        index: 15,
      },
    ]
  },
  {                                                             //17
    url: "../images/comp/ne-crd-3.jpg",
    infospots: [
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [-4949.47, 146.69, 631.97],
        title: "Praça D'Armas",
        index: 18,
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [4952.01, 321.04, -572.18],
        title: "Voltar",
        index: 16,
      },
    ]
  },
  {                                                             //18
    url: "../images/comp/ne-crd-5.jpg",
    infospots: [
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [-3667.97, 904.50, 3268.33],
        title: "Praça D'Armas",
        index: 19,
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [4345.92, 2451.12, -146.08],
        title: "Voltar",
        index: 17,
      },
    ]
  },
  {                                                             //19
    url: "../images/comp/ne-crd-6.jpg",
    infospots: [
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [-4798.48, 49.95, -1376.29],
        title: "Praça D'Armas",
        index: 20,
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [4437.84, 404.22, 2251.60],
        title: "Voltar",
        index: 18,
      },
    ]
  },
  {                                                             //20
    url: "../images/comp/ne-crd-7.jpg",
    infospots: [
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [-4943.14, 499.31, -529.44],
        title: "Praça D'Armas",
        index: 21,
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [4963.13, -251.23, 515.09],
        title: "Voltar",
        index: 19,
      },
    ]
  },
  {                                                             //21
    url: "../images/comp/ne-crd-8.jpg",
    infospots: [
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [315.05, -1788.17, 4649.09],
        title: "SSTT",
        index: 22,
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [-4741.02, 1567.42, 114.86],
        title: "Praça D'Armas",
        index: 32,
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [-271.11, 456.09, -4962.83],
        title: "Voltar",
        index: 20,
      },
    ]
  },
  {                                                             //22
    url: "../images/comp/ne-crd-9.jpg",
    infospots: [
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [4302.05, 2080.61, 1450.44],
        title: "SSTT",
        index: 23,
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [-4598.42, -662.26, 1821.75],
        title: "Voltar",
        index: 21,
      },
    ]
  },
  {                                                             //23
    url: "../images/comp/ne-crd-10.jpg",
    infospots: [
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [508.20, -1766.11, 4645.02],
        title: "Salas de Aula",
        index: 24,
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [2469.79, -104.45, -4340.92],
        title: "SSTT",
        index: 40,
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [-4027.21, -2350.30, 1796.77],
        title: "Voltar",
        index: 22,
      },
    ]
  },
  {                                                             //24
    url: "../images/comp/ne-crd-11.jpg",
    infospots: [
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [4218.46, 1772.26, -1996.50],
        title: "Sala de Instrução a Navegação",
        index: 25,
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [4990.00, 233.98, 15.36],
        title: "SSTT",
        index: 40,
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [4774.68, -384.62, -1414.38],
        title: "Voltar",
        index: 22,
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [-2900.78, 1105.11, -3914.33],
        title: "Sala de Aula",
        index: 44,
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [-3299.23, 1758.85, 3305.17],
        title: "Sala de Aula",
        index: 44,
      },
    ]
  },
  {                                                             //25
    url: "../images/comp/ne-crd-12.jpg",
    infospots: [
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [552.01, -16.30, 4967.31],
        title: "Passadiço",
        index: 26,
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [3133.23, -104.61, 3880.18],
        title: "Sala de Instrução a Navegação",
        index: 47,
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [1291.26, -93.36, -4819.50],
        title: "Câmara do Comandante",
        index: 45,
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [-2157.70, -4505.49, -53.39],
        title: "Voltar",
        index: 24,
      },
    ]
  },
  {                                                             //26
    url: "../images/comp/ne-crd-13.jpg",
    infospots: [
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [2708.57, 3353.69, -2521.96],
        title: "Passadiço",
        index: 27,
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [-326.15, 374.77, 4968.18],
        title: "Sala de Instrução a Navegação",
        index: 47,
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [-1575.59, 1707.22, -4417.82],
        title: "Sala de Instrução a Navegação",
        index: 49,
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [4796.01, 1226.78, -648.52],
        title: "Câmara do Comandante",
        index: 45,
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [4153.86, 122.14, -2762.63],
        title: "Voltar",
        index: 24,
      },
    ]
  },
  //PASSADIÇO
  {                                                             //27
    url: "../images/comp/ne-pass-1.jpg",
    infospots: [
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [-3190.20, -188.44, 3832.09],
        title: "Passadiço",
        index: 28,
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [-3157.19, -2270.69, -3133.96],
        title: "Voltar",
        index: 26,
      },
    ]
  },
  {                                                             //28
    url: "../images/comp/ne-pass-2.jpg",
    infospots: [
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [4702.64, 1487.13, 769.31],
        title: "",
        index: 29,
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [4828.61, 554.86, -1137.73],
        title: "",
        index: 30,
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [-1030.78, 47.23, 4890.25],
        title: "Voltar",
        index: 27,
      },
    ]
  },
  {                                                             //29
    url: "../images/comp/ne-pass-3.jpg",
    infospots: [
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [-4900.07, -875.32, 378.58],
        title: "",
        index: 30,
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [-4872.65, 1052.59, 228.91],
        title: "Mastro",
        index: 0,
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [3327.51, -2070.17, 3094.29],
        title: "",
        index: 28,
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [4831.33, -1041.22, 692.71],
        title: "Voltar",
        index: 27,
      },
    ]
  },
  {                                                             //30
    url: "../images/comp/ne-pass-4.jpg",
    infospots: [
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [4988.01, -195.61, 82.07],
        title: "",
        index: 29,
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [-3645.91, 982.18, -3271.05],
        title: "Mastro",
        index: 0,
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [4367.84, -1067.08, 2165.92],
        title: "",
        index: 28,
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [4844.24, -742.89, 969.83],
        title: "Voltar",
        index: 27,
      },
    ]
  },
  //CAMAROTE DE GM
  {                                                             //31
    url: "../images/comp/ne-cam-1.jpg",
    infospots: [
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [2754.15, -133.36, 4158.84],
        title: "Voltar",
        index: 16,
      },
    ]
  },
  //PRAÇA D'ARMAS DOS OFICIAIS
  {                                                             //32
    url: "../images/comp/ne-prd-1.jpg",
    infospots: [
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [-1074.72, -939.24, 4785.37],
        title: "",
        index: 33,
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [-4800.62, 1201.51, -664.88],
        title: "Voltar",
        index: 21,
      },
    ]
  },
  {                                                             //33
    url: "../images/comp/ne-prd-2.jpg",
    infospots: [
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [4106.43, -1088.96, 2619.70],
        title: "",
        index: 34, //sofa
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [3245.89, 180.53, -3787.40],
        title: "",
        index: 35, //mesa
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [1135.73, 503.04, -4835.48],
        title: "",
        index: 36, //bar
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [-4949.95, 516.13, 381.26],
        title: "Voltar",
        index: 21,
      },
    ]
  },
  {                                                             //34
    url: "../images/comp/ne-prd-3.jpg",
    infospots: [
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [-4981.08, 214.31, -200.30],
        title: "",
        index: 35, 
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [-4438.51, -94.77, 2279.10],
        title: "",
        index: 36, 
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [-2725.22, -433.13, 4159.76],
        title: "",
        index: 33, 
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [-84.24, -83.47, 4992.31],
        title: "Voltar",
        index: 21,
      },
    ]
  },
  {                                                             //35
    url: "../images/comp/ne-prd-5.jpg",
    infospots: [
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [4994.83, -60.33, -53.33],
        title: "",
        index: 34, 
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [-279.84, -121.96, 4980.54],
        title: "",
        index: 36, 
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [3715.94, -418.03, 3316.10],
        title: "",
        index: 33, 
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [4483.16, -132.67, 2188.18],
        title: "Voltar",
        index: 21,
      },
    ]
  },
  {                                                             //36
    url: "../images/comp/ne-prd-4.jpg",
    infospots: [
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [-4828.35, -1201.74, -453.50],
        title: "",
        index: 35, 
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [-2205.84, -127.91, -4474.38],
        title: "",
        index: 34, 
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [-691.36, -160.11, -4939.53],
        title: "",
        index: 33, 
      },
    ]
  },
  //ATENDIMENTO ODONTOLÓGICO
  {                                                             //37
    url: "../images/comp/ne-dent-1.jpg",
    infospots: [
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [1283.62, 909.06, -4735.61 ],
        title: "Voltar",
        index: 15, 
      },
    ]
  },
  //CENTRO CIRÚRGICO
  {                                                             //38
    url: "../images/comp/ne-cir-1.jpg",
    infospots: [
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [-4958.12, 435.92, -406.56],
        title: "",
        index: 39, 
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [-3917.40, 684.28, -3017.31],
        title: "Voltar",
        index: 15, 
      },
    ]
  },
  {                                                             //39
    url: "../images/comp/ne-cir-2.jpg",
    infospots: [
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [-4966.69, -235.60, 458.94],
        title: "",
        index: 38, 
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [781.58, 386.74, 4916.23],
        title: "Voltar",
        index: 15, 
      },
    ]
  },
  //SSTT
  {                                                             //40
    url: "../images/comp/ne-sstt-1.jpg",
    infospots: [
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [-4918.32, 803.60, -302.72],
        title: "",
        index: 41, 
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [-3995.76, 1564.17, -2553.26],
        title: "Cubículo",
        index: 42, 
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [4979.48, 106.93, 311.37],
        title: "Voltar",
        index: 24, 
      },
    ]
  },
  {                                                             //41
    url: "../images/comp/ne-sstt-2.jpg",
    infospots: [
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [3989.15, 2989.19, -311.29],
        title: "Cubículo",
        index: 42, 
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [2188.18, 1245.60, 4309.39],
        title: "Voltar",
        index: 24, 
      },
    ]
  },
  {                                                             //42
    url: "../images/comp/ne-sstt-3.jpg",
    infospots: [
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [3460.88, 2306.46, 2763.14],
        title: "",
        index: 43, 
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [-2460.76, 1223.26, -4172.30],
        title: "Voltar",
        index: 41, 
      },
    ]
  },
  {                                                             //43
    url: "../images/comp/ne-sstt-4.jpg",
    infospots: [
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [-3275.28, 23.26, -3771.54],
        title: "",
        index: 42, 
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [-1306.49, 380.19, -4803.77],
        title: "Voltar",
        index: 41, 
      },
    ]
  },
  //SALA DE AULA
  {                                                             //44
    url: "../images/comp/ne-sala-1.jpg",
    infospots: [
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [-2557.26, 985.05, 4171.59],
        title: "Voltar",
        index: 24, 
      },
    ]
  },
  //CÂMARA DO COMANDANTE
  {                                                             //45
    url: "../images/comp/ne-cmdt-1.jpg",
    infospots: [
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [1805.32, -172.91, -4648.12],
        title: "",
        index: 46, 
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [4543.63, 665.06, -1963.55],
        title: "Voltar",
        index: 25, 
      },
    ]
  },
  {                                                             //46
    url: "../images/comp/ne-cmdt-2.jpg",
    infospots: [
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [-264.54, 233.32, 4976.97],
        title: "",
        index: 45, 
      },
    ]
  },
  //SALA DE INSTRUÇÃO A NAVEGAÇÃO
  {                                                             //47
    url: "../images/comp/ne-nav-1.jpg",
    infospots: [
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [4518.07, 784.85, 1990.03],
        title: "",
        index: 48, 
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [-4973.76, 433.17, -114.33],
        title: "Voltar",
        index: 26, 
      },
    ]
  },
  {                                                             //48
    url: "../images/comp/ne-nav-2.jpg",
    infospots: [
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [-4612.50, 354.73, 1878.20],
        title: "",
        index: 49, 
      },
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [402.57, 419.53, -4960.27],
        title: "Voltar",
        index: 47, 
      },
    ]
  },
  {                                                             //49
    url: "../images/comp/ne-nav-3.jpg",
    infospots: [
      {
        icon:'%PUBLIC_URL%/arrow.png',
        position: [2571.69, 895.21, 4183.50],
        title: "Voltar",
        index: 47, 
      },
    ]
  },
  // Add more panoramas here if needed  
];

export default panoramasData;